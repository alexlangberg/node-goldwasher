...
var getOptions = function(options) {
  var schema = Joi.object().keys({
    ...
    format: Joi.string().valid('json', 'xml', 'rss'),
    href: Joi.boolean(),
...
var getMetaTagList = function($, name) {
  var list = getMetaTag($, name).split(',');
  return R.map(function(item) {
    return S(item).trim().s;
  }, list);
};
...
var convertToRss = function(nuggets, meta) {
  var feed = new RSS({
    author: meta.author,
    categories: meta.categories,
    custom_namespaces: meta.custom_namespaces,
    description: meta.description,
    docs: meta.custom_namespaces.goldwasher,
    feed_url: meta.feedUrl,
    generator: 'goldwasher with RSS for Node',
    pubDate: meta.time,
    site_url: meta.url,
    title: meta.title
  });
  ...
  R.forEach(function(item) {
    var keyWords = R.map(function(keyWord) {
      return keyWord.word;
    }, item.keywords);
    feed.item({
      title:  item.text,
...
var getMeta = function($, options) {
  return {
    ...
    categories: getMetaTagList($, 'keywords'),
    custom_namespaces: {
      goldwasher: 'https://github.com/alexlangberg/node-goldwasher'
    },
...
var goldwasher = function(html, userOptions) {
  ...
  if (options.format === 'rss') {
    return convertToRss(nuggets, meta);
  }
