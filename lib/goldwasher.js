 ...
 * @returns {boolean}
 */
var nuggetFilter = function(text, stopTexts, index, limit, search) {
  if (limit && index >= limit) { return true; }

  if (R.difference([text], stopTexts).length === 0) { return true; }
...
var inputFromCheerio = function($, options) {
  ...
  scraped.each(function(index) {
    ...
    if (simpleText === '' || simpleText === null) { return; }
