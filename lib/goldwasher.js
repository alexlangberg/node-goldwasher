 ...
 * }}
 */
var inputFromFeed = function(feed, options) {
...
var detectInputType = function(input) {
  if (R.is(String, input)) {
    ...
    // detect if HTML. Lamest detector ever?
    if (S(input).contains('<')) {
      return 'html';
    }
...
var goldwasher = function(input, userOptions) {
  ...
  if (options.input === 'cheerio' ||
    ...
    if (options.input !== 'cheerio') {
      data = cheerio.load(input);
    } else {
      data = input;
    }

    batch = inputFromCheerio(data, options);
  }

  if (options.input === 'feed') {
    data = new NodePie(input);
    data.init();

    batch = inputFromFeed(data, options);
  }

  // now that we have the nuggets, determine how to output them
